...
func NewMDNSListener(cache *ServiceCache, parser *DaprMDNSParser, appIDPrefix string) (*MDNSListener, error) {
...
func NewRefresher(cache *ServiceCache, parser *DaprMDNSParser, interval time.Duration) *Refresher {
	ctx, cancel := context.WithCancel(context.Background())
	return &Refresher{
		cache:    cache,
		parser:   parser,
...
func (r *Refresher) refresh() {
	...
	for _, appID := range appIDs {
		...
		if len(services) > 0 {
			...
			//	r.cache.AddOrUpdate(appID, service)
			//}
...
func captureMDNSQueries(cache *ServiceCache, parser *DaprMDNSParser, appIDPrefix string, ctx context.Context) {
	// 获取多播接口列表
	...
	go func() {
		defer wg.Done()
		captureMDNSQueriesIPv4(cache, parser, appIDPrefix, ctx, interfaces)
	}()

	go func() {
		defer wg.Done()
		captureMDNSQueriesIPv6(cache, parser, appIDPrefix, ctx, interfaces)
	}()
...
// captureMDNSQueriesIPv4 捕获 IPv4 mDNS 查询
func captureMDNSQueriesIPv4(cache *ServiceCache, parser *DaprMDNSParser, appIDPrefix string, ctx context.Context, interfaces []net.Interface) {
	// mDNS 通配符地址（用于绑定）
	addr := &net.UDPAddr{
	...
	if err != nil {
		log.Printf("无法绑定 IPv4 mDNS 通配符地址: %v", err)
		return
	}

	defer udpConn.Close()

	// 包装为 ipv4.PacketConn 以加入多播组
	...
	log.Println("IPv4 mDNS 查询捕获器已启动")

	// 处理数据包
	processPackets(cache, parser, appIDPrefix, ctx, udpConn, "IPv4")
}

// captureMDNSQueriesIPv6 捕获 IPv6 mDNS 查询
func captureMDNSQueriesIPv6(cache *ServiceCache, parser *DaprMDNSParser, appIDPrefix string, ctx context.Context, interfaces []net.Interface) {
	// mDNS 通配符地址（用于绑定）
	addr := &net.UDPAddr{
		IP:   net.ParseIP("ff02::"),
		Port: 5353,
	...
	if err != nil {
		log.Printf("无法绑定 IPv6 mDNS 通配符地址: %v", err)
		return
	}

	defer udpConn.Close()

	// 包装为 ipv6.PacketConn 以加入多播组
	...
	log.Println("IPv6 mDNS 查询捕获器已启动")

	// 处理数据包
	processPackets(cache, parser, appIDPrefix, ctx, udpConn, "IPv6")
}

// processPackets 通用数据包处理逻辑
func processPackets(cache *ServiceCache, parser *DaprMDNSParser, appIDPrefix string, ctx context.Context, conn net.PacketConn, protocol string) {
	buf := make([]byte, 65536)
	dnsMsg := new(dns.Msg)
	...
	for {
		...
		default:
			...
			for _, q := range dnsMsg.Question {
				serviceName := q.Name
				// 提取 AppID
				appID := ExtractAppIDFromServiceName(serviceName)
				...
				//log.Printf("捕获到 %s mDNS 查询: 服务名称=%s, AppID=%s", protocol, serviceName, appID)
			}
